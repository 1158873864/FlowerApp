...
public class TokenBlServiceImpl implements TokenBlService {
    ...
    public TokenAddResponse addToken(TokenAddParameters tokenAddParameters) {
        Token token = tokenDataService.saveToken(new Token(TokenState.INACTIVE, tokenAddParameters.getCredits(), tokenAddParameters.getMinLimit(), tokenAddParameters.getPrice(), tokenAddParameters.getStartDate(), tokenAddParameters.getEndDate(), userDataService.getUserByUsername(UserInfoUtil.getUsername())));
